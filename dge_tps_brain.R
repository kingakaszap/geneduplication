...
# import data & merge ----
# 1 - htseqcount data
F_B_1029<- read.table("msc/firststep/data/htseq_tps_brain/Tps_F_B_Ad_18-1029.txt", stringsAsFactors=F,header=F,sep="\t")
F_B_1029 <- F_B_1029 %>% rename(Gene = V1, Count = V2) %>% 
  mutate(sample = "F_B_1029")

F_B_1030 <- read.table("msc/firststep/data/htseq_tps_brain/Tps_F_B_Ad_18-1030.txt", stringsAsFactors=F,header=F,sep="\t")
F_B_1030 <- F_B_1030 %>% rename(Gene = V1, Count = V2) %>% 
  mutate(sample = "F_B_1030" )

F_B_1031 <- read.table("msc/firststep/data/htseq_tps_brain/Tps_F_B_Ad_18-1031.txt", stringsAsFactors=F,header=F,sep="\t")
F_B_1031 <- F_B_1031 %>% rename(Gene = V1, Count = V2) %>% 
  mutate(sample = "F_B_1031" )

F_B_1041 <- read.table("msc/firststep/data/htseq_tps_brain/Tps_F_B_Ad_18-1041.txt", stringsAsFactors=F,header=F,sep="\t")
F_B_1041 <- F_B_1041 %>% rename(Gene = V1, Count = V2) %>% 
  mutate(sample = "F_B_1041")

M_B_1033 <- read.table("msc/firststep/data/htseq_tps_brain/Tps_M_B_Ad_18-1033.txt", stringsAsFactors=F,header=F,sep="\t")
M_B_1033 <- M_B_1033 %>% rename(Gene = V1, Count = V2) %>% 
  mutate(sample ="M_B_1033")

M_B_1035 <- read.table("msc/firststep/data/htseq_tps_brain/Tps_M_B_Ad_18-1035.txt", stringsAsFactors=F,header=F,sep="\t")
M_B_1035 <- M_B_1035 %>% rename(Gene = V1, Count = V2) %>% 
  mutate(sample = "M_B_1035")

M_B_1036 <- read.table("msc/firststep/data/htseq_tps_brain/Tps_M_B_Ad_18-1036.txt", stringsAsFactors=F,header=F,sep="\t")
M_B_1036 <- M_B_1036 %>% rename(Gene = V1, Count = V2) %>% 
  mutate(sample = "M_B_1036")

M_B_1042 <- read.table("msc/firststep/data/htseq_tps_brain/Tps_M_B_Ad_18-1042.txt", stringsAsFactors=F,header=F,sep="\t")
M_B_1042 <- M_B_1042 %>% rename(Gene = V1, Count = V2) %>% 
  mutate(sample = "M_B_1042")

# combine df-s
list_of_dfs <- list(F_B_1029, F_B_1030,F_B_1031,F_B_1041,M_B_1033,M_B_1035,M_B_1036, M_B_1042)
all_counts_brain <- list_of_dfs %>%
  bind_rows() %>%  
...
expr <- DGEList(counts=all_counts_brain)
...
expressed_genes <- all_counts_brain$Gene
...
write.table(rpkm_norm_df, file="msc/firststep/rpkm_brain.txt",quote=F, sep="\t")
...
counts_filtered_brain <- as.data.frame(all_counts_brain[all_counts_brain$Gene %in% rpkm_filtered$Gene, ])
write.table(counts_filtered_brain, file="msc/firststep/data/filtered_counts_brain.txt",quote=F, sep="\t")

## table on whether genes are DE or not ----
# set gene column as rownames again
rownames(counts_filtered_brain) <- counts_filtered_brain$Gene
counts_filtered_brain <- counts_filtered_brain %>% select(-Gene)
...
# Create edgeR object which will contain count data and normalization factors
expr <- DGEList(counts=counts_filtered_brain, group=conditions)
# Calculate normalization factors that we need to account for
norm_expr <- calcNormFactors(expr)
...
table_brain <- et$table
# table$Padj <- p_FDR original code but this wasnt defined before so i assume its p_adj?
table_brain$Padj <- p_adj
table_brain$Gene  <- rownames(table_brain) # gene column back again
write.table(table_brain, file="msc/firststep/data/differential_expression_brain.txt",quote=F, sep=",")

## DE stats on all genes----

# % of all genes that are DE
diff_expr_genes_brain<- table_brain %>% filter (Padj <= 0.05)
nrow(diff_expr_genes_brain)/nrow(table_brain) # 0.2 % ??

# add sex-biased/not
table_brain<- table_brain %>% 
...
# Bias type 
table_brain$sex_bias <- "unbiased"
# if log2Foldchange > 1 and p < 0.05, male biased
table_brain$sex_bias[table_brain$logFC > 1 & table_brain$Padj < 0.05] <- "M"
# if log2Foldchange < -1 and p < 0.05, female biased
table_brain$sex_bias[table_brain$logFC < 1 & table_brain$Padj < 0.05] <- "F"

sum_for_volcano <- table_brain %>% 
  group_by(sex_bias) %>% 
  summarise(count = n()) %>% 
  mutate(percentage = round((count / nrow(table_brain)) * 100, 1))
sum_for_volcano
# Volcano plot
(volcanoplot_brain <- ggplot(table_brain, aes(x=logFC, y = -log10(Padj), col = sex_bias))+
    geom_point(size = 0.7)+
    theme_classic()+
   theme(legend.position = "none",
    ...
    ylim(c(-8, 60))+
    xlim(c(-10,10))+
    ggtitle("d) Brain (n = 7746)\n")+
    guides(color = guide_legend(title = "Expression pattern", override.aes = list(size = 5)))+
...
ggsave("msc/firststep/plots/tps_volcano_brain.png", width = 8, height =6, dpi = 600)
# should I change the axes (make it more zoomed in?)
## duplicate expression patterns ----
# filter duplicates that are expressed
duplicates_comparable_brain <- duplicate_pairs %>% filter (Duplicate.1 %in% table_brain$Gene & Duplicate.2 %in% table_brain$Gene)
nrow(duplicates_comparable_brain)
# 2653

# what proportion of duplicates are expressed in antenna
nrow(duplicates_comparable_brain)/nrow(duplicate_pairs) # 26%

# what proportion of expressed genes are dups
nrow(duplicates_comparable_brain)/nrow(table_brain) #35.6%

# what proportion of dups are DE
diff_expr_duplicates_brain <- diff_expr_genes_brain %>%
  filter(Gene %in% duplicates_comparable_brain$Duplicate.1 | Gene %in% duplicates_comparable_brain$Duplicate.2)
nrow(diff_expr_duplicates_brain)/nrow(duplicates_comparable_brain)
# 0.4 % are sex-biased

# what proportion of DE genes are duplicates
nrow(diff_expr_duplicates_brain)/nrow(diff_expr_genes_brain) # 68%

# what proportion of unbiased genes are duplicates
(nrow(duplicates_comparable_brain)- nrow(diff_expr_duplicates_brain))/(nrow(table_brain)- nrow(diff_expr_genes_brain))
# 35.5%


# what proportion of singletons are DE
singletons <- table_brain %>% filter (duplicate_status == "singleton")
(singletons_summary  <- singletons %>% 
    group_by(diff_exp_sexes) %>% 
    summarise(count = length(diff_exp_sexes)))
3/(3+3520) # basically 0

# add new column to dups dataframe,: is duplicate 1 sex-biased? is duplicate 2 sex-biased?
duplicates_comparable_brain<- duplicates_comparable_brain %>% mutate( 
  Dup1_de = ifelse(Duplicate.1 %in% diff_expr_genes_brain$Gene, "yes", "no"),
  Dup2_de = ifelse (Duplicate.2 %in%diff_expr_genes_brain$Gene, "yes", "no"))

# and specific pattern
duplicates_comparable_brain <- duplicates_comparable_brain %>% 
...
# Add logfc values
logfc_to_add <- diff_expr_genes_brain %>% select(logFC, Gene) %>% rename(Duplicate.1 = Gene)
duplicate_pairs_logfc <- left_join(duplicates_comparable_brain, logfc_to_add, by = "Duplicate.1")
...
duplicates_comparable_brain <- duplicate_pairs_logfc_both
...
(discordant_summary <- duplicates_comparable_brain %>% 
    group_by(discordant) %>% 
    summarise(count = length(discordant)))
16/(16+2637)
# 0.06 % show discordant expression

# pattern in transposed genes
transposed_exp <- duplicates_comparable_brain %>% filter(duplication_type == "transposed") %>% 
  filter(pattern == 2)
...
duplicate_proportions_expressed_brain <- duplicates_comparable_brain %>% 
  group_by(duplication_type) %>%
  summarise(percentage = 100* n() / nrow(duplicate_pairs))
# Plot proportions
(expressed_duplicates_barplot_brain <- ggplot(duplicate_proportions_expressed_brain, aes(x = duplication_type, y = percentage, fill = duplication_type)) +
    geom_bar(stat = "identity") +  
    labs(x = "\nDuplication type", y = "% of all pairs\n") +
    scale_fill_brewer(palette = "Paired") +
    theme_classic())
ggsave("msc/firststep/plots/duplication_types_proportions_brain.png", dpi = 500)

# 2 - basic patterns in duplicates that are expressed
duplicates_comparable_brain$pattern<- as.factor( duplicates_comparable_brain$pattern)
duplicates_comparable_brain$duplication_type<- as.factor(duplicates_comparable_brain$duplication_type)
(basic_plot <- ggplot (duplicates_comparable_brain, aes(x = pattern, fill = duplication_type))+
    geom_bar(stat = "count")+
    labs(x = "\nPattern", y = "Count\n")+
...
duplicates_percent_brain <- duplicates_comparable_brain %>%
  group_by(bias_pattern, duplication_type) %>%
  summarise(count = n(), .groups = "drop") %>%
  mutate(percent = 100 * count / sum(count))  

# Plot
(expression_patterns_plot_brain <- ggplot(duplicates_percent_brain, aes(x = bias_pattern, y = percent, fill = duplication_type)) +
    ...
    theme(legend.position = "none",
          ...
          plot.title = element_text(size = 13)))
ggsave("msc/firststep/plots/tps_brain_expression_patterns_dups.png", dpi=600)

(duplication_types_plot_brain <- ggplot(duplicates_percent_brain, aes(x = duplication_type, y = percent, fill = bias_pattern)) +
    geom_bar(stat = "identity") +  # Use calculated percentages
    labs(x = "\nPattern", y = "Percentage (%)") +
    scale_fill_brewer(palette = "Paired") +
    guides(fill = guide_legend(title = "Duplication type")) +
    theme_classic())
ggsave("msc/firststep/plots/tps_duplication_types_expression_brain.png", dpi=600)

# pie
duplicates_for_pie_brain <- duplicates_percent_brain %>%
  group_by(bias_pattern) %>%
  summarise(percent = sum(percent))
View(duplicates_for_pie_brain)
# Reorder the bias_pattern factor
duplicates_for_pie_brain$bias_pattern <- factor(
  duplicates_for_pie_brain$bias_pattern, 
  levels = c("UU", "FF", "MM", "UM", "UF", "FM")  # Specify the desired order
)

(pie_brain <- ggplot(duplicates_for_pie_brain, aes(x = "", y = percent, fill = bias_pattern))+
    geom_bar(stat = "identity", width = 1)+
    coord_polar(theta = "y")+
    labs (x = NULL, y = NULL, title = "d) Brain")+
    geom_text(aes(label = ifelse(bias_pattern %in% ("UU"), 
                                 paste0(bias_pattern, "\n", round(percent, 1), "%"), NA)),
...
# chisq tests - probably need to do fishers, do later ----
# Paralog/singleton, sex-biased/unbiased
# step 1 - add duplicate/not duplicate status to counts_filtered
counts_filtered_brain$Gene <- rownames(counts_filtered_brain)

counts_filtered_brain <- counts_filtered_brain %>% mutate(
  duplicate = ifelse((Gene %in% duplicate_pairs$Duplicate.1 | Gene %in% duplicate_pairs$Duplicate.2),
                     "duplicated", "not_duplicated"),
  de = ifelse ((Gene %in% diff_expr_genes_brain$Gene), "sex-biased", "unbiased"))
...
# venn -----
View(table_brain)
duplicated_genes <- table_brain$Gene[table_brain$duplicate_status == "paralog"]
sex_biased_genes <- table_brain$Gene[table_brain$diff_exp_sexes == "sex-biased"]
length(duplicated_genes) # 3923
length(sex_biased_genes) # 16
# Define total counts
all_genes <- nrow(table_brain)
windows()

png("msc/firststep/plots/venn_diagram_antenna.png", width = 950, height = 600)
grid.newpage()
...
counts_filtered_brain <- counts_filtered_brain%>% mutate(
  duplicate = ifelse((Gene %in% duplicate_pairs$Duplicate.1 | Gene %in% duplicate_pairs$Duplicate.2),
                     "duplicated", "not_duplicated"),
  de = ifelse ((Gene %in% diff_expr_genes_brain$Gene), "sex-biased", "unbiased"))
chisq_data <- counts_filtered_brain %>% select(duplicate,de)
chisq_matrix <- table(chisq_data$duplicate, chisq_data$de)
chisq.test(chisq_matrix)
fisher.test(chisq_matrix)
chisq_matrix
chisq.test(chisq_matrix)$expected
chisq.test(chisq_matrix)$residuals


chisq_data_4 <- duplicates_comparable_brain %>% 
  select(duplication_type, discordant)
chisq_matrix_4 <- table(chisq_data_4$duplication_type, chisq_data_4$discordant)
fisher.test(chisq_matrix_4)
fisher.test(chisq_matrix_4)$expected
fisher.test(chisq_matrix_4)$residuals
